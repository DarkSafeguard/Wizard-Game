...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		else // no head hit, check all other boxes
		{
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	while (Older->GetValue().Time > HitTime) // is Older still younder than HitTime?
	{
...
void ULagCompensationComponent::ServerScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
	...
	{
		const float Damage = Confirm.bHeadShot ? Character->GetEquippedWeapon()->GetDamage() * Character->GetEquippedWeapon()->GetHeadshotDamageMultiplier() : Character->GetEquippedWeapon()->GetDamage();
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		...
		{
			float HeadShotDamage = Confirm.HeadShots[HitCharacter] * (HitCharacter->GetEquippedWeapon()->GetDamage() * HitCharacter->GetEquippedWeapon()->GetHeadshotDamageMultiplier());
