...
//Client SeamlessTravel Fix

void ABlasterPlayerController::NotifyLoadedWorld(FName WorldPackageName, bool bFinalDest)
{
...
bool ABlasterPlayerController::ServerNotifyLoadedWorldWorkaround_Validate(FName WorldPackageName)
{
	RPC_VALIDATE(WorldPackageName.IsValid());
	return true;
}

void ABlasterPlayerController::ServerNotifyLoadedWorldWorkaround_Implementation(FName WorldPackageName)
{
	UE_LOG(LogPlayerController, Verbose, TEXT("ABlasterPlayerController::ServerNotifyLoadedWorldWorkaround_Implementation: Client loaded %s"), *WorldPackageName.ToString());
...
void ABlasterPlayerController::HighPingWarning()
{
	...
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
		BlasterHUD->CharacterOverlay->PingAlertIcon &&
		BlasterHUD->CharacterOverlay->HighPingAnimation;
	if (bHUDValid)
	{
		//BlasterHUD->CharacterOverlay->PingAlertIcon->SetOpacity(1.f);
...
void ABlasterPlayerController::StopHighPingWarning()
{
	...
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->PingAlertIcon->SetOpacity(0.f);
...
FString ABlasterPlayerController::GetInfoText(const TArray<class ABlasterPlayerState*>& Players)
{
	...
	{
		InfoTextString = Announcement::TiedForTheWin;
...
FString ABlasterPlayerController::GetTeamsInfoText(ABlasterGameState* BlasterGameState)
{
	...
	{
		InfoTextString = TeamAnnouncement::ThereIsNoWinner;
	}
	else if (RedTeamScore == BlueTeamScore)
	{
		InfoTextString = FString::Printf(TEXT("%s\n"), *TeamAnnouncement::TeamsTied);
		InfoTextString.Append(FString::Printf(TEXT("%s\n%s"), *TeamAnnouncement::RedTeam, *TeamAnnouncement::BlueTeam));
	}
	else if (RedTeamScore > BlueTeamScore)
	{
		InfoTextString = TeamAnnouncement::RedTeamWins;
	}
	else if (RedTeamScore < BlueTeamScore)
	{
		InfoTextString = TeamAnnouncement::BlueTeamWins;
	}
